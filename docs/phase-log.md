# ExecutiveOS — Phase log

> The authoritative, phase-by-phase record: what was built, what broke, what review found, and what
> was validated live. Split out of `CLAUDE.md` on 2026-07-27, when that file crossed Claude Code's
> **150k-char auto-load limit** (and a headless `claude-9arm` was dying with
> `ContextWindowExceededError` at 99k input tokens before doing any work).
>
> **New phase entries go here.** `CLAUDE.md` keeps only the index table + the standing rules;
> `HANDOFF.md` is the cold-start doc; `GOTCHA.md` collects the traps.
